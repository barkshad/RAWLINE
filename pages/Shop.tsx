            ...
            <motion.div key={product.id} variants={FADE_UP}>
              <HoverCard>
                <Link to={`/product/${product.handle}`} className="flex flex-col">
                  <div className="aspect-[3/4] overflow-hidden mb-10 bg-charcoal/5 relative transition-all duration-700">
                    <motion.img 
                      whileHover={IMAGE_FOCUS.hover}
                      transition={IMAGE_FOCUS.transition}
                      src={getCloudinaryUrl(product.images[0], { width: 1000 })} 
                      alt={product.title}
                      className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-[1200ms]"
                    />
